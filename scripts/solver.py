    ...
    def test_attack(self):
        ...
        for i, (x_real, c_org) in enumerate(data_loader):
            ...
            for idx, c_trg in enumerate(c_trg_list):
                ...
                out_path_adv = os.path.join(self.result_dir, "x_adv.jpg")  # 保存中间结果到output文件夹
                utils.save_image(
                    # q: x_adv[0]是什么？
                ...
                # Metrics
                with torch.no_grad():
                    # q: 这里的self.G（）对于完全相同的输入，输出是否相同？
                    # a: 不同，因为G中有dropout，每次输出都不同
                    # q: 请解释G中的dropout，它有什么功能？
                    # a: dropout是一种正则化方法，它在训练过程中以一定的概率将网络中的神经元丢弃，这样可以减少神经元之间的依赖关系，防止过拟合。
                    # q: dropout的代码在哪里？
                    # a: 在model.py中的Generator类中，有一个nn.Dropout2d()，它是一个二维dropout，它的功能是以一定的概率将输入张量中的元素置为0。
