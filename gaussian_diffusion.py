...
class Solver(object):
    ...
    def test_model_result(self, model_output, celeba, ccount):
        """Translate images using StarGAN trained on a single dataset."""
        self.restore_model(self.test_iters)
        data_loader = self.celeba_loader
        # Load the trained generator.
        img_t = Image.open('img.jpg')
        img_tensor = np.array(img_t)
        #img_tensor = img_tensor.to(self.device)
        # img, label = celeba[ccount]
        loss_t = 1
        with (torch.no_grad()):
            for i, (x_real, c_org) in enumerate(data_loader):
                if i == ccount:
                    ...
                    label = c_trg_list[0]
                    x_f = self.G(model_output, label)
                    loss_t = mean_flat((x_f - img_tensor) ** 2)
...
class GaussianDiffusion:
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            # print(config)
            global ccount
            terms["g_1"] = init_stargan(config, model_output, ccount)
            ccount += 1

            if "vb" in terms:
                terms["loss"] = terms["mse"] + terms["g_1"] + terms["vb"]
            else:
                terms["loss"] = terms["mse"] + terms["g_1"]
        else:
...
def init_stargan(config, model_output, ccount):
    rafd_loader = None

    celeba_loader, celeba = get_loader(config.celeba_image_dir, config.attr_path, config.selected_attrs,
                               config.celeba_crop_size, config.image_size, config.batch_size,
                               'CelebA', config.mode, config.num_workers)

    solver = Solver(celeba_loader, rafd_loader, config)
    v = solver.test_model_result(model_output, celeba, ccount)
    return v
