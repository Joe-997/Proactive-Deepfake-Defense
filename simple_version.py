# parameters --> data_loader(celeba) --> solver --> solver.test_attack()
from model_s import Generator
from model_s import Discriminator
...
class Solver(object):
    ...
    def test_attack(self):
        ...
        for i, (x_real, c_org) in enumerate(data_loader):
            ...
            pgd_attack = attacks_s.LinfPGDAttack(model=self.G, device=self.device, feat=None)
            ...
            for idx, c_trg in enumerate(c_trg_list):
                ...
                x_adv_list.append(x_adv)   # 这里就是生成的对抗样本

                '''
                # 1. 保存perturb到图像文件
                if i == 0:    # 0是降噪生成结果
                    path = os.path.join(self.result_dir, f'{idx}perturb-image.jpg')
                    save_image(self.denorm(perturb.data.cpu()), path, nrow=1, padding=0)
                '''
                ...
                with torch.no_grad():
                    ...
                    # 2. 保存特定图片，对抗样本
                    if i == 0:
                        path = os.path.join(self.result_dir, f'{c}adv-image.jpg')
                        save_image(self.denorm(x_adv.data.cpu()), path, nrow=1, padding=0)
                        c += 1
                    '''
